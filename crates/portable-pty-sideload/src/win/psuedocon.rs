...
/// (nexterm.exe on Windows).  Used as the anchor for sideloaded
/// `conpty.dll` + `OpenConsole.exe`.  Falls back to `None` if we
/// cannot determine the exe path (extremely unusual but possible in
/// embedded / service contexts).
fn current_exe_dir() -> Option<std::path::PathBuf> {
    std::env::current_exe().ok().and_then(|p| p.parent().map(|d| d.to_path_buf()))
}

/// NexTerm-local patch (upstream psmux refuses to sideload because it
/// runs *inside* other terminals).  We are the hosting terminal, so we
/// ship Microsoft's `conpty.dll` + `OpenConsole.exe` alongside the
/// binary and use them in preference to the system `kernel32.dll`
/// implementation.
///
/// Why: Windows 11 Insider/Canary builds (notably 26200) accept
/// `PSEUDOCONSOLE_PASSTHROUGH_MODE` at `CreatePseudoConsole` time but
/// then reject the resulting HPCON from `CreateProcessW` with
/// `ERROR_INVALID_PARAMETER (87)`.  The stock `kernel32.dll` ConPTY
/// hits that bug, which forces the fork's `new_without_passthrough`
/// retry path and silently disables passthrough.  Microsoft's
/// standalone `conpty.dll` (same binary Windows Terminal uses) does
/// not exhibit the bug, so passthrough actually sticks and we stop
/// paying the VT re-parse cost on scroll-heavy workloads.
///
/// Lookup order:
///   1. `$NEXTERM_CONPTY_DLL` — explicit override, for tests / devs.
///   2. `<exe dir>/conpty.dll` — bundled with the release build.
///   3. `kernel32.dll` — same as upstream, graceful degradation if
///      no sideloaded runtime is present.
///
/// `OpenConsole.exe` is looked up implicitly by `conpty.dll` itself
/// via its normal DLL search rules; placing it next to `conpty.dll`
/// is sufficient.
fn load_conpty() -> ConPtyFuncs {
    if let Ok(override_path) = std::env::var("NEXTERM_CONPTY_DLL") {
        match ConPtyFuncs::open(Path::new(&override_path)) {
            Ok(f) => {
                log::info!(
                    "sideloaded ConPTY via NEXTERM_CONPTY_DLL = {}",
                    override_path
                );
                return f;
            }
            Err(e) => log::warn!(
                "NEXTERM_CONPTY_DLL={} could not be loaded ({:?}); \
                 falling back to <exe>/conpty.dll + kernel32.dll",
                override_path,
                e,
            ),
        }
    ...
    if let Some(exe_dir) = current_exe_dir() {
        let side_dll = exe_dir.join("conpty.dll");
        if side_dll.exists() {
            match ConPtyFuncs::open(&side_dll) {
                Ok(f) => {
                    log::info!(
                        "sideloaded ConPTY from {}",
                        side_dll.display()
                    );
                    return f;
                }
                Err(e) => log::warn!(
                    "failed to sideload {}: {:?}; falling back to kernel32.dll",
                    side_dll.display(),
                    e,
                ),
            }
    ...
    log::info!("using system kernel32.dll ConPTY (no sideloaded conpty.dll found)");
